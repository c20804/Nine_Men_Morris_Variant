...
class ABGame:
    ...
    def ABMidgame(self, board):
        alpha = float('-inf')
        beta = float('inf')
        bestScore = float('-inf')
        bestboard = board
        possibleBoards = self.generateMove(board, True)
        for b in possibleBoards:
            score = self.alphaBeta(b, 1, alpha, beta, False)
    ...
    def alphaBeta(self, board, d, alpha, beta, maximizingPlayer):
        ...
        if maximizingPlayer:
            value = float('-inf')
            for b in possibleBoards:
                value = max(value, self.alphaBeta(b, d + 1, alpha, beta, False))
                if value >= beta:
                    break
            ...
            value = float('inf')
            for b in possibleBoards:
                value = min(value, self.alphaBeta(b, d + 1, alpha, beta, True))
                if value <= alpha:
                    break
                beta = min(beta, value)
            return value
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 4:
        print("Usage: python ABGame.py <input> <output> <depth>")
        sys.exit(1)
    else:
    ...
    game = ABGame(board, depth)
    bList, bestScore = game.ABMidgame(board)
